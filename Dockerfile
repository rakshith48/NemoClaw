...
# Non-secret flag: set to "1" when the user configured a web-search provider
# during onboard. Controls whether the web search block is written to
# openclaw.json. The actual API key is injected at runtime via
# openshell:resolve:env, never baked into the image.
ARG NEMOCLAW_WEB_SEARCH_ENABLED=0
# Non-secret: which web-search provider the user selected during onboard
...
RUN set -eu; \
    ...
    if [ "$NEMOCLAW_WEB_SEARCH_ENABLED" = "1" ]; then \
        if [ "$NEMOCLAW_WEB_SEARCH_PROVIDER" = "firecrawl" ]; then \
            openclaw plugins install "npm:@openclaw/firecrawl-plugin@${OPENCLAW_VERSION}" --pin; \
            FIRECRAWL_API_KEY=openshell:resolve:env:FIRECRAWL_API_KEY openclaw doctor --fix --non-interactive; \
        else \
